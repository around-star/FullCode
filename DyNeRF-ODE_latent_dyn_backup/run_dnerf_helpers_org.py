...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, num_frames, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
        ...
        #self.latent_output_size = self.latent_time_net.output_size
        self.latent_output_size = latent_dim
        self._time, self._time_out = self.create_time_net()
        #self.latent_fc = nn.Linear(131072, 512)
    ...
    def query_time(self, new_pts, t, net, net_final, latent):

        #time_h = self.latent_time_net(t.int()).float()
        #time_h = self.latent_fc(torch.unsqueeze(latent, 0))
        time_h = torch.unsqueeze(latent, dim=0)
        time_h = time_h.repeat(new_pts.shape[0], 1)
    ...
    def forward(self, x, ts, latent=None):
        ...
        if cur_time == 0. and self.zero_canonical:
            ...
            dx = self.query_time(input_pts, ts, self._time, self._time_out, latent)
            input_pts_orig = input_pts[:, :3]
            input_pts = self.embed_fn(input_pts_orig + dx)
        out, _ = self._occ(torch.cat([input_pts, input_views], dim=-1), t)
        return out, dx
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    # Invert CDF
    u = u.contiguous()
    inds = searchsorted(cdf.cpu(), u.cpu(), side='right').cuda()
