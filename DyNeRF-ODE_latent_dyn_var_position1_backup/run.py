...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i in range(1):
        ...
        print(i)
        print(times_to_pred)
        latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.16509287059307098, 0.8372400403022766, 0.10000000149011612])
...
def train():
    ...
    """nerf_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position1/logs_ball_roll_together_diff_starting_points5/ss_100_n_obs_1_15m_loc_mlp_latent_nODE_loc_sup_l2_no_warmup_no_relu_last_div_grad_align_pen_250_after_5k_wt_decrease_abs/pitcher_base/070000_old.tar')
    ...
    #div_latents_target = np.load('latent_mlp_500_100.npy')
    #div_target = np.load('divergence_mlp_500_100.npy')
    #div_target = torch.tensor(div_target.mean(0))[1:]
    for i in trange(start, N_iters):
        ...
        if i < 0:
            latents, seen, latents_angle, angle_to_pred = vid_ode_model(batch_dict, warmup=True)
            total_loss = 0
            optimizer.zero_grad()
            #seen = 0
            ...
            #print(pose)
            #target = target[seen]  ## THIS
            #pose = pose[seen, :3, :4]  ## THIS
        else:
            if i > 1000:
            #if i < 0:
                latents_full, seen, latents_angle, angle_to_pred, div_pred = vid_ode_model(batch_dict, freeze = False, div=True)
            else:
                latents_full, seen, latents_angle, angle_to_pred, div_pred = vid_ode_model(batch_dict, freeze = False)
        ...
        if not latents_angle is None:
            #angle_loss = 0.01*l1_loss(latents_angle, angle_to_pred)
            angle_loss = 0.01*torch.mean((latents_angle - angle_to_pred)**2)
            #angle_loss = 0.01*cos_sim(latents_angle, angle_to_pred, torch.ones(latents_angle.shape[0]))
        #total_loss += angle_loss
        #if not div_pred is None:
        if i > 1000:
        #if i<0:
        #    div_loss = 0.00001*torch.mean((div_pred - div_target)**2)
            #div_loss = 10*div_pred.sum()#.abs()


            latents_grad = torch.gradient(latents_full, axis=0)[0]

            div_pred = latents_grad.sum(-1)  ## Different divergence

            latents_grad_norm = latents_grad / torch.norm(latents_grad, dim=-1, keepdim=True)
            latents_full_norm = latents_full / torch.norm(latents_full, dim=-1, keepdim=True)

            grad_alignment = torch.squeeze(latents_grad_norm * latents_full_norm)
            grad_alignment = grad_alignment.sum(-1)

            wt_grad_alignment = div_pred * grad_alignment
            
            div_loss = 5e-4*wt_grad_alignment.sum(dim=0).abs()

            #geom_mean_loss = (loss * div_loss)**(1/2)
            #wt_div_loss = get_loss_weight(i, 80000)
            #div_loss = wt_div_loss*wt_grad_alignment.sum(dim=0).abs()



        #total_loss += div_loss
        #total_loss = angle_loss + geom_mean_loss
        total_loss = angle_loss + loss + div_loss
        #total_loss = (angle_loss*loss)**(1/2) + div_loss
        #optimizer.zero_grad()
        ...
        if i % args.i_print == 0:                
            ...
            if angle_loss:
                #tqdm_txt += f" Angle Loss: {angle_loss.item()}"
                tqdm_txt += " Angle Loss: {:.6f}".format(angle_loss.item())
            if div_loss:
                #tqdm_txt += f" Div Loss: {div_loss.item()}"
                tqdm_txt += " Div Loss: {:.6f}".format(div_loss.item())

            if geom_mean_loss:
                #tqdm_txt += f" Geom Mean Loss: {geom_mean_loss.item()}"
                tqdm_txt += " Geom Mean Loss: {:.6f}".format(geom_mean_loss.item())
        ...
        if i%args.i_video==0 and i >= 10000:
        #if True:
