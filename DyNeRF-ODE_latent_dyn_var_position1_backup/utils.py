...
def split_data_extrap(data_dict, opt):
    ...
    split_dict = {"observed_data": data_dict["data"][:, start_obs:start_obs+n_observed_tp].clone(),
                  ...
                  "data_to_predict": torch.cat((data_dict["data"][:, :start_obs].clone(), data_dict["data"][:, start_obs+n_observed_tp_pred:].clone()), dim=1),
                #   "data_to_predict": data_dict["data"][:, start_obs+n_observed_tp:].clone(),
                  "tp_to_predict": torch.cat((data_dict["time_steps"][:start_obs].clone(), data_dict["time_steps"][start_obs+n_observed_tp_pred:].clone()), dim=0),
    ...
    if ("poses" in data_dict) and (data_dict["poses"] is not None):
        ...
        split_dict["poses_to_pred"] = torch.cat((data_dict["poses"][:, :start_obs].clone(), data_dict["poses"][:, start_obs+n_observed_tp_pred:].clone()), dim=1)
    ...
    if ("times" in data_dict) and (data_dict["times"] is not None):
        ...
        split_dict["times_to_pred"] = torch.cat((data_dict["times"][:, :start_obs].clone(), data_dict["times"][:, start_obs+n_observed_tp_pred:].clone()), dim=1)
    ...
    if ("angle" in data_dict) and (data_dict["angle"] is not None): 
        ...
        split_dict["angle_to_pred"] = torch.cat((data_dict["angle"][:, :start_obs].clone(), data_dict["angle"][:, start_obs+n_observed_tp_pred:].clone()), dim=1)
