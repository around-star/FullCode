...
def create_LatentODE_model(input_dim, z0_prior, obsrv_std, device, 
    ...
    if poisson:
        ...
        #dim = latents ### OG Code
        #latents_new = latents
        #latents_new = latents + angle_dim
        latents_new = latents + 512
        linear = None
    ...
    elif z0_encoder == "mlp_2":
        layers = [nn.Linear(enc_input_dim, 256)]
        layer = nn.Linear
        layers += [layer(256, 128)]
    ...
    elif z0_encoder == "mlp_3":
        layers = [nn.Linear(enc_input_dim, 64)]
        layer = nn.Linear
        layers += [layer(64, 32)]
        layers += [layer(32, 8)]
    ...
    elif z0_encoder == "mlp_4":
        W = 256
        layers = [nn.Linear(enc_input_dim, 128)]
        layers += [nn.Linear(128, W)]
        for i in range(6):
