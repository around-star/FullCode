...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, latent_store, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    #for j, frame_time in enumerate(render_times[3:]):
        ...
        latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs),
                                                torch.tensor(time_to_pred), angle=[-0.4351, 0.6672, 0.10000000149011612])
        ...
        #for j, latent_next in enumerate(torch.unsqueeze(latent_nexts, 0)):
...
def train():
    ...
    if args.render_only or True:
        ...
        nerf_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position1/logs_ball_roll_together_diff_starting_points5/same_loc_mlp_latent_nODE_loc_sup_l2_no_warmup_no_relu_last_decoder_add_latent_div_grad_align_pen_250_wt_1e-4_abs_1k/pitcher_base/070000.tar')
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_loc_extrap500_-0.4351_0.6672_70ckpt'.format('test' if args.render_test else 'path', start))
            #testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_latent_orgresult_75k'.format('test' if args.render_test else 'path', start))
