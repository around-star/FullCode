...
def train():
    ...
    load_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position1/logs_ball_roll_together_diff_starting_points4/ss_100_euler_all_pred_n_obs_1_15motions_loc_mlp_latent_nODE_loc_supervised_l2_no_warmup_no_relu_last_layer_div_grad_align_pen_250_wt_1e-3_abs/pitcher_base/070000.tar')#["network_fine_state_dict"]
    ...
    #time_obs.register_hook(lambda grad: grads.update({'time_obs': grad}))
    #angles.register_hook(lambda grad: grads.update({'angles': grad}))
    ...
    x_meshgrid = np.linspace(min(tsne_result[:,0]), max(tsne_result[:,0]), 100)
    y_meshgrid = np.linspace(min(tsne_result[:,1]), max(tsne_result[:,1]), 100)
    ...
    plt.savefig('tsne_dense_ball_roll_loc_mlp_latent_nODE_loc_supervised_l2_no_warmup_no_relu_last_layer_div_pen_grad_align_250_wt_1e-3_abs/nODE_10000_grid_100_locs_exrap_250_50_torchautograd_div_contf_norm_add1e-4_70k_ckpt.png')
