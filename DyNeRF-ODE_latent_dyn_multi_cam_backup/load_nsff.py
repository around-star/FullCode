...
def load_llff_data(args, basedir, start_frame, end_frame, 
                   factor=8, target_idx=10, 
                   recenter=True, bd_factor=.9, 
                   spherify=False, path_zflat=False, 
                   final_height=288, skip=1, device='cuda'):
    
    poses, bds, imgs, = _load_data(basedir, 
                                    start_frame, end_frame,
                                    height=final_height, skip=skip, 
                                    evaluation=False)
    print('Loaded', basedir, bds.min(), bds.max())
    ...
    times = np.tile(np.linspace(0.,1.,args.sample_size), 10)
