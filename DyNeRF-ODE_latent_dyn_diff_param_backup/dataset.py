...
class Dataset_base(Dataset):
    ...
    def sample_regular_extrap(self, images, poses, times, damping):
        ...
        #win_start = 100*random.randint(0,700/100-1)
        win_start = 80*random.randint(0,560/80-1)
        # win_start = 0
        ...
        poses = poses[win_start: win_start + self.sample_size]
        times = times[win_start: win_start + self.sample_size]
        damping = damping[win_start: win_start + self.sample_size]
        mask = torch.ones((self.sample_size, 1))
        mask = mask.type(torch.FloatTensor).to(self.device)
        
        return input_images, poses, times, damping, mask, win_start/100
    ...
    def sampling(self, images, poses, times, damping):
        
        # Sampling
        if not self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_regular_interp(images=images, poses=poses, times=times, damping=damping)
        elif not self.irregular and self.opt.extrap:
            input_images, poses, times, damping, mask, win_start = self.sample_regular_extrap(images=images, poses=poses, times=times, damping=damping)
        elif self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_irregular_interp(images=images, poses=poses, times=times, damping=damping)
        else:
            input_images, poses, times, mask = self.sample_irregular_extrap(images=images, poses=poses, times=times, damping=damping)
        
        return input_images, poses, times, damping, mask, win_start
...
class VideoDataset(Dataset_base):
    ...
    def __init__(self, opt, images=None, poses=None, times=None, damping=None, train=True):
    ...
    def __getitem__(self, index):
        ...
        # Sampling
        input_images, poses, times, damping, mask, win_start = self.sampling(images=images, poses=poses, times=times, damping=damping)

        # Transform
        #input_images = self.vtrans(input_images)  # return (b, c, h, w)

        return torch.tensor(input_images).float(), torch.tensor(poses).float(), torch.tensor(times).float(), torch.tensor(damping), torch.tensor(mask).float(), torch.tensor(win_start)
    
    def __len__(self):
        return len(self.images)


def parse_datasets(opt, imgs, poses, times, damping, device):
    def video_collate_fn(batch, time_steps, opt=opt, data_type="train"):
        ...
        damping = torch.stack([b[3] for b in batch])
        mask = torch.stack([b[4] for b in batch])
        win_start = torch.stack([b[5] for b in batch])
        
        data_dict = {"data": images, "poses":poses, "times": times, "time_steps": time_steps, "damping": damping, "mask": mask, "win_start":win_start}
    ...
    train_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, damping, train=True),
                                      batch_size=opt.batch_size,
                                      shuffle=True,
                                      collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="train"),
                                      generator=torch.Generator(device='cuda'))
    test_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, damping, train=False),
                                     batch_size=opt.batch_size,
                                     shuffle=False,
                                     collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="test"),
                                     generator=torch.Generator(device='cuda'))
