...
def get_next_batch(data_dict, test_interp=False):
    ...
    if ("damping" in data_dict) and (data_dict["damping"] is not None):
        batch_dict["damping"] = data_dict["damping"]
        batch_dict["damping_to_pred"] = data_dict["damping_to_pred"]
...
def split_data_extrap(data_dict, opt):
    ...
    if ("damping" in data_dict) and (data_dict["damping"] is not None): 
        split_dict["damping"] = data_dict["damping"][:, start_obs:start_obs+n_observed_tp].clone()
        split_dict["damping_to_pred"] = torch.cat((data_dict["damping"][:, :start_obs].clone(), data_dict["damping"][:, start_obs+n_observed_tp:].clone()), dim=1)
