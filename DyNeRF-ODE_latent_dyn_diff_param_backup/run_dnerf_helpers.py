...
class LatentNetwork(nn.Module):
    def __init__(self, input_size=1, latent_size=512, output_size=8):
        super(LatentNetwork, self).__init__()
        self.latent_size = latent_size
        self.output_size = output_size
        #self.fc = nn.Linear(input_size, latent_size)
        self.fc = nn.Embedding(input_size, latent_size)
        self.std = 0.01/(self.latent_size ** (1/2))
        #self.std = 0.1
...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, num_frames, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, zero_canonical=True, latent_dim=512):
        ...
        #self.latent_time_net = LatentNetwork()
        #self.latent_time_net = LatentNetwork(input_size=num_frames)
        #self.latent_output_size = self.latent_time_net.latent_size
        self.latent_output_size = 512
        self._time, self._time_out = self.create_time_net()
        self._occ = NeRFOriginal(D=D, W=W, input_ch=input_ch, 
                                 input_ch_views=input_ch_views,
                                 latent_dim=self.latent_output_size,
    ...
    def create_time_net(self):
        layers = [nn.Linear(self.input_ch + self.latent_output_size, self.W)]
    ...
    def query_time(self, new_pts, t, net, net_final):
        ...
        time_h = self.latent_time_net(t.int()).float()
        h = torch.cat([new_pts, time_h], dim=-1)
        for i, l in enumerate(net):
            h = net[i](h)
            h = F.relu(h)
            if i in self.skips:
    ...
    def forward(self, x, ts, latent, interp=False):
        ## print("X: ", x.shape) ## [32000, 90]
        input_pts, input_views = torch.split(x, [self.input_ch, self.input_ch_views], dim=-1)
        ## print("Input Pts: ", input_pts.shape) ## [32000, 63]
        t = ts[0]
        ...
        """if not interp:
            ...
            time_h = self.latent_time_net(ts.int()).float()
        else:
            print("Interpolation")
            time_h1 = self.latent_time_net(ts.int()).float()
            time_h2 = self.latent_time_net(ts.int() + 1).float()
            time_h = (time_h1 + time_h2)/2"""
        time_h = torch.unsqueeze(latent, dim=0)
        #print(time_h.shape)
        #print(input_pts.shape)
        #exit()
        time_h = time_h.repeat(input_pts.shape[0], 1)
        out, _ = self._occ(torch.cat([input_pts, input_views], dim=-1), time_h)
        return out
...
class NeRFOriginal(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, latent_dim = 512, input_ch_time=1, output_ch=4, skips=[4],
        ...
        layers = [nn.Linear(input_ch+latent_dim, W)]
        for i in range(D - 1):
            if i in memory:
                raise NotImplementedError
            else:
        ...
        self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W + latent_dim, W//2)])
    ...
    def forward(self, x, ts):
        ...
        if self.use_viewdirs:
            ...
            h = torch.cat([feature, input_views, ts], -1)
