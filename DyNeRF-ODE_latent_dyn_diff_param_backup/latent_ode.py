...
class LatentODE(nn.Module):
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, latent_truth_time_steps,
        latent_time_steps_to_pred=None, damping=None, mask = None, n_traj_samples = 1, run_backwards = True, mode = None,
        warmup=False):
        if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
            ...
            #if damping:
            damping = self.embed_damp(damping[0])
            #print(damping.shape)
            #damping = damping.repeat(2,1)
            latent_embeddings = torch.cat([damping, latent_embeddings], dim=-1)
    ...
    def forward(self, batch_dict, warmup=False):
        ...
        #batch_dict["times"] = ((self.num_frames-1)*batch_dict["times"].to(self.device)).int()
        #batch_dict["times_to_pred"] = ((self.num_frames-1)*batch_dict["times_to_pred"].to(self.device)).int()
        damping = batch_dict["damping"].to(self.device)
        #print(damping)
    ...
    def next_latent(self, latent, times_obs, times_to_pred, damping=0.2, run_backwards=True, n_traj_samples=1):
        ...
        if latent==None:
            ...
            damping = self.embed_damp(torch.tensor([damping]))
            latent = torch.cat([damping, latent], dim=-1)
