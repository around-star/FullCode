...
def train():
    ...
    load_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_diff_param/logs_ball_roll_together_diff_param/sample_size_80_euler_all_pred_warmup_n_obs_1/pitcher_base/180000.tar')#["network_fine_state_dict"]
    ...
    dampings = [0.03, 0.08, 0.15, 0.2, 0.25, 0.4]
    times = torch.arange(0,200)/100
    time_obs = times[:1]
    time_to_pred = times
    latents, _ = latent_ode_model.next_latent(None,
                                            torch.tensor(time_obs),
                                            torch.tensor(time_to_pred), damping = dampings[0])
    
    all_latents = latents.cpu().detach().numpy()
    for damping in dampings[1:]:
        
        latents, _ = latent_ode_model.next_latent(None,
                                            torch.tensor(time_obs),
                                            torch.tensor(time_to_pred), damping = damping)
    ...
    for i in range(6):
        ...
        print(i)
        cmap = create_colormap(c[i], 200)
        plt.scatter(tsne_result[i*200:(i+1)*200, 0], 
                    tsne_result[i*200:(i+1)*200, 1], 
    ...
    legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor='grey', markersize=10, label='{} unseen'.format(dampings[0])),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='green', markersize=10, label='{}'.format(dampings[1])),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='{} unseen'.format(dampings[2])),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='purple', markersize=10, label='{}'.format(dampings[3])),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='pink', markersize=10, label='{} unseen'.format(dampings[4])),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='{}'.format(dampings[5]))
    ]
    plt.title('t-SNE Visualization')
    plt.xlabel('Dimension 1')
    plt.ylabel('Dimension 2')
    plt.legend(handles=legend_elements, loc="best")
    plt.savefig('tsne_diff_param_ball_roll/nODE_200_timesteps.png')
