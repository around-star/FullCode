...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, latent_store, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    times = torch.arange(0,250)/100
    time_obs = times[:1]
    #time_to_pred = render_times*5
    time_to_pred = times[1:][::2]
    c2w = render_poses[0]
    for i in range(1):
    ...
    #for j, frame_time in enumerate(render_times[3:]):
        ...
        latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs),
                                                torch.tensor(time_to_pred), damping=0.35)
...
def train():
    ...
    #with open('Blender_data_big_pendulum_damp_diff_param/transforms_train.json', 'r') as fp:
    #    transforms = json.load(fp)
    #render_poses = torch.tensor(np.array(transforms['frames'][0]['transform_matrix'])).float()
    #img = imageio.imread('Blender_data_big_pendulum_damp_diff_param/'+transforms['frames'][0]['file_path']+'.png')
    ...
    if args.render_only or True:
        ...
        nerf_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_diff_param/logs_ball_roll_together_diff_param/sample_size_80_euler_all_pred_warmup_n_obs_1/pitcher_base/180000.tar')
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_damp_0.35_extrap'.format('test' if args.render_test else 'path', start))
