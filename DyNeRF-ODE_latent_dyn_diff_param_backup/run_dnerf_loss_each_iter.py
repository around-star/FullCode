...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(args, render_poses, render_times, hwf, latent_store, args.chunk, render_kwargs_test, gt_imgs=images,
    ...
    vid_ode_model = VidODE(args, device)
    latent_store = {}
    for frame in range(args.num_frames+1):
        latent_store.update({str(frame): torch.zeros([131072])})
    ...
    for i in trange(start, N_iters):
        ...
        latents = vid_ode_model(batch_dict)
        #rgbs = []
        #optimizer.zero_grad()
        ...
        for i_idx in range(len(all_targets)):
            ...
            if args.add_tv_loss:
                ...
                latent_next = latents[i_idx + 1] if i_idx < len(latents) else None
                ...
                if latent_prev is not None:
                    ...
                    _, _, _, extras_prev = render(H, W, focal, latent=latent_prev, chunk=args.chunk, rays=batch_rays, frame_time=int(rand_time_prev*args.num_frames),
                                                    verbose=i < 10, retraw=True, z_vals=extras['z_vals'].detach(),
                                                    **render_kwargs_train)
                ...
                if latent_next is not None:
                    ...
                    #                                verbose=i < 10, retraw=True, z_vals=extras['z_vals'].detach(),
                    #                                **render_kwargs_train)
                    _, _, _, extras_next = render(H, W, focal, latent=latent_next, chunk=args.chunk, rays=batch_rays, frame_time=int(rand_time_next*args.num_frames),
                                                    verbose=i < 10, retraw=True, z_vals=extras['z_vals'].detach(),
                                                    **render_kwargs_train)
            ...
            new_lrate_embedding = 0.5 * args.lrate * (decay_rate ** (global_step / decay_steps))
            for param_i, param_group in enumerate(optimizer.param_groups):
                if param_i == (len(optimizer.param_groups) - 1):
        ...
        if i%args.i_weights==0:
            ...
            latent_path = os.path.join(basedir, expname, '{:06d}'.format(i))
            latent_file = open(latent_path, 'ab')
            save_dict = {
